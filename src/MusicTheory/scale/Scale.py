...
class Scale:
    def __init__(self, keyName='C', intervals=ScaleIntervals.Major.Ionian):
...
#        self.__calcPitchClasses()
    ...
    def __ValidateInterval(self, v):
        if not issubclass(v.__class__, Intervals): raise TypeError(f'引数intervalsはenum型Intervals継承クラスのいずれかにしてください。: type(v)={type(v)}')
