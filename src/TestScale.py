...
#        self.assertEqual(ScaleIntervals.Major.Aeolian, s.Intervals)
    ...
    def test_init_set_Invalid_Key(self):
        with self.assertRaises(ValueError) as ex:
            s = Scale('無効値', ScaleIntervals.Major.Aeolian)
#            s = Scale('無効値', ScaleIntervals.Major.Aeolian)
        self.assertIn('keyは次のうちのいずれかにしてください。', str(ex.exception))
    def test_init_set_Invalid_Interval(self):
        with self.assertRaises(TypeError) as ex:
            s = Scale('C', '無効値')
        self.assertIn('引数intervalsはenum型Intervals継承クラスのいずれかにしてください。', str(ex.exception))
...
#        print('※※※ScaleKeyが変更されてもScaleに通知されず構成音の再計算がされない！※※※')
    def test_Intervals_Set(self):
        # C Minor Scale
        s = Scale()
#        s.Intervals = ScaleIntervals.Major.Aeolian
#        self.assertEqual(ScaleIntervals.Major.Aeolian, s.Intervals)
        s.Intervals = ScaleIntervals.Major.Aeolian
        self.assertEqual(ScaleIntervals.Major.Aeolian, s.Intervals)
            ...
            for acc_count in range(0, 2):
                for a, ap in Accidental.Accidentals.items():
                    ...
                    with self.subTest(name=name):
                        s = Scale(name, intervals=ScaleIntervals.Major.Ionian)
        ...
        for k, kp in keys.items():
            for acc_count in range(0, 2):
                for a, ap in Accidental.Accidentals.items():
                    name = k + a*acc_count
                    pitch = PitchClass.Get(kp + (ap*acc_count))[0]
                    intervals = ScaleIntervals.Major.Ionian
...
#                    print()
    ...
    def test_Names(self):
        ...
        s = Scale()
        s.Key.Name = 'C'
        s.Intervals = ScaleIntervals.Major.Ionian
